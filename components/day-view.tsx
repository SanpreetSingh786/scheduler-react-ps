...
interface DayViewProps {
  ...
  timeScale?: number
  timeOffset?: number
  onTimeScaleWheel?: (e: React.WheelEvent) => void
}
...
export function DayView({
  ...
  const handleAppointmentDoubleClick = useCallback(
    (appointment: Appointment) => {
                  ...
                  {appointmentLanes.map((lane, laneIndex) =>
                    lane.map((appointment) => {
                      ...
                      return (
                        <div
                          key={appointment.id}
                          className={`absolute rounded px-3 py-2 text-xs text-white font-medium ${appointment.color} cursor-move hover:opacity-80 transition-all duration-200 shadow-md border-l-4 border-white ${isDragging ? "opacity-50 shadow-lg scale-105" : ""}`}
                          style={{
                            ...style,
